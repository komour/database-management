...
-- забронированные билеты, могут быть оплачены либо нет
CREATE TABLE Booked(
    UserId int NOT NULL,
...
FROM ValidFlights NATURAL JOIN Flights NATURAL JOIN Booked
WHERE Paid OR                         -- reedemed tickets
...
$func$
BEGIN
   INSERT INTO Booked (UserId, FlightId, SeatId, Expiration, Paid)
...
$func$
BEGIN
   INSERT INTO Booked (UserId, FlightId, SeatId, Paid)
...
BEGIN
   UPDATE Booked
   SET Expiration = NULL, Paid = TRUE, UserId = uId
   WHERE
       UserId = uId AND
       FlightId = fId AND
       SeatId = sId AND
